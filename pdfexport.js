...
async function exportToPDF() {
    try {
        ...
        if (transactions.length === 0) {
            alert("Tidak ada data transaksi yang ditemukan!");
            return;
        }
        ...
        // Save PDF
        doc.save(`Laporan_Keuangan_${formatDate(minDate)}_${formatDate(maxDate)}.pdf`);
...
function addHeader(doc, userData, minDate, maxDate) {
    ...
    doc.text(`No. Rekening: ${userData.accountNumber}`, 15, 45);
...
function addFooter(doc, userData) {
    ...
    doc.text(`Nasabah: ${userData.name}`, 15, 280);
    doc.text(`No. Rek: ${userData.accountNumber}`, 15, 285);
